...
export function ContactForm({ lang, dict }: { lang: Locale; dict: Dictionary }) {
  ...
  if (state?.ok) {
    return (
      <motion.div
        ...
        className="surface flex flex-col items-center rounded-[var(--radius-card)] p-10 text-center"
      >
  ...
  return (
    <form action={action} className="surface rounded-[var(--radius-card)] p-7 sm:p-8">
      ...
      {state && !state.ok && (
        <p className="mt-3 text-sm text-red-700">{dict.contact.errorGeneric}</p>
      )}
